...
export class LoanFormComponent implements OnInit, OnDestroy{
  ...
  constructor(private fb: FormBuilder) {
    ...
    this.loanForm = this.fb.group({
      ...
      termYears: ['', [Validators.required, Validators.min(1), Validators.max(30)], ]
    });
    this.loanForm.get('totalAmount')!.valueChanges.subscribe(totalAmount => {
      ...
      if (totalAmount >= 1000) {
        ...
        downPaymentControl?.setValidators([Validators.required, Validators.min(totalAmount * 0.15), Validators.max(totalAmount * 0.90)]);
      } else {
